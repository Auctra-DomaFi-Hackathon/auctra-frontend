...
import { Coins, TrendingUp, ArrowRight, AlertCircle, CheckCircle, Loader2 } from 'lucide-react'
...
export default function SupplyPanel({ className }: SupplyPanelProps) {
  ...
  const [currentAction, setCurrentAction] = useState<'approve' | 'supply' | 'withdraw' | null>(null)

  // Check if approval is needed
  useEffect(() => {
    if (supplyAmount && usdcAllowance !== undefined && typeof usdcAllowance === 'bigint') {
  ...
  const handleMaxWithdraw = () => {
    if (userPosition.shares && poolData.exchangeRate) {
      const maxWithdrawable = (userPosition.shares * poolData.exchangeRate) / BigInt('1000000000000000000')
  ...
  const getSupplyAPR = () => {
    ...
    return (borrowRate * utilizationRate * (1 - reserveFactor)).toFixed(1) + '%'
  }

  const getUserSuppliedAmount = () => {
    if (userPosition.shares && poolData.exchangeRate) {
      const suppliedAmount = (userPosition.shares * poolData.exchangeRate) / BigInt('1000000000000000000')
        ...
        {isSupplyMode ? (
            ...
            <div className="space-y-3">
              {needsApproval && (
                <Button 
                  ...
                  disabled={!supplyAmount || (isPending && currentAction === 'approve') || (isConfirming && currentAction === 'approve')}
                  className="w-full"
                  variant="outline"
                >
                  {isPending && currentAction === 'approve' || isConfirming && currentAction === 'approve' ? (
                    <>
              ...
              <Button
                ...
                disabled={!supplyAmount || (isPending && currentAction === 'supply') || (isConfirming && currentAction === 'supply') || needsApproval}
                className="w-full"
              >
                {isPending && currentAction === 'supply' || isConfirming && currentAction === 'supply' ? (
                  <>
                    <Loader2 className="h-4 w-4 mr-2 animate-spin" />
          ...
          <div className="space-y-4">
            ...
            <Button
              ...
              disabled={!withdrawAmount || (isPending && currentAction === 'withdraw') || (isConfirming && currentAction === 'withdraw')}
              className="w-full"
            >
              {isPending && currentAction === 'withdraw' || isConfirming && currentAction === 'withdraw' ? (
                <>
                  <Loader2 className="h-4 w-4 mr-2 animate-spin" />
